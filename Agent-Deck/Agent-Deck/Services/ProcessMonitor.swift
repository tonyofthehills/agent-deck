...
class ProcessMonitor: ObservableObject {
    ...
    private func createOrUpdateInstance(pid: pid_t, detectedInstances: inout [AgentInstance]) -> AgentInstance? {
        ...
        if var existingInstance = instances.first(where: { $0.pid == pid }) {
            ...
            if let task = currentTask, task != existingInstance.currentTask {
                ...
                // Task completed - set to done status
    ...
    /// Filters out Claude desktop app (Claude.app) helper processes
        ...
        do {
            ...
            if let output = String(data: data, encoding: .utf8) {
                for line in output.split(separator: "\n") {
                    ...
                    // Also filter out node processes related to Claude desktop app
                    if commandLine.contains("node") && commandLine.contains("claude.app") {
                        continue
                    }
        ...
        do {
            ...
            for projectDir in projectDirs {
                ...
                if FileManager.default.fileExists(atPath: transcriptPath.path) {
                    ...
                    Logger.info("Transcript cwd: \(workingDir)", log: Logger.monitoring)

                    // Find instance with matching working directory
                    // Match if instance's working directory starts with or equals the project path
                    if let index = instances.firstIndex(where: {
                        $0.workingDirectory == workingDir || $0.workingDirectory.hasPrefix(workingDir + "/") || workingDir.hasPrefix($0.workingDirectory + "/")
                    }) {
